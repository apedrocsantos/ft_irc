...
class Server
{
    ...
    std::map<int, char *> in_buf; //buffer with incomplete input messages, indexed by the client fd
    std::map<int, std::string> out_buf; //buffer with output messages, indexed by the client fd
    void receive_msg();
    void send_msg();
    ...
    std::map<int, char *>& get_buf() {return in_buf;};
	...
	void add_to_out_buf(int fd, std::string msg) {out_buf[fd].append(msg);};
};
