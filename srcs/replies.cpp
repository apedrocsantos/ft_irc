...
void ERR_NickCollision(Client *client) {
	std::string output = std::string(":ircserv ") + "436" + " " + client->getNick() + " :Nickname collision KILL from " + client->getUsername() + "@" + client->getHostname() + "\r\n";
	send(client->getFd(), output.c_str(), output.size(), 0);
}

void ERR_NickNameInUse(Client *client) {
	std::string output = std::string(":ircserv ") + "433" + " " + client->getNick() + " :Nickname is already in use\r\n";
	send(client->getFd(), output.c_str(), output.size(), 0);
}

void ERR_ErroneusNickName(Client *client) {
	std::string output = std::string(std::string(":ircserv ")) + "432" + " " + client->getNick() + " :Erroneus nickname\r\n";
	send(client->getFd(), output.c_str(), output.size(), 0);
}

void ERR_NoNicknameGiven(Client *client) {
	std::string output = std::string(":ircserv ") + "431" + " " + client->getNick() + " :No nickname given\r\n";
	send(client->getFd(), output.c_str(), output.size(), 0);
}

void ERR_AlreadyRegistered(Client *client) {
	std::string output = std::string(":ircserv ") + "462" + " " + client->getNick() + " :You may not reregister\r\n";
	send(client->getFd(), output.c_str(), output.size(), 0);
}
